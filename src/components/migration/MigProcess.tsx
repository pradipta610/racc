import { MIG_PROCESS_STEPS } from "@/lib/data";

export default function MigProcess() {
  return (
    <section style={{ padding: "72px 5%", background: "var(--white)" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          ...
          }}>Our 4-Step Migration Process</h2>
          <p style={{ fontSize: 15, color: "var(--tm)", maxWidth: 600, margin: "0 auto", lineHeight: 1.7 }}>
            We make the complex simple. Here&apos;s how we take you from first enquiry to visa approval.
          </p>
        </div>

        <div style={{
          display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 0, position: "relative",
        }} className="mig-psteps">
          <div className="mig-psteps-line" />
          {MIG_PROCESS_STEPS.map((s) => (
      ...
      <style>{`
        .mig-psteps-line {
          position: absolute;
          top: 34px; left: 12.5%; width: 75%;
          height: 2px; background: var(--border); z-index: 0;
        }
        @media (max-width: 768px) {
          .mig-psteps { grid-template-columns: 1fr 1fr !important; gap: 24px !important; }
          .mig-psteps-line { display: none !important; }
        }
        @media (max-width: 480px) {
          .mig-psteps { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </section>
  );
